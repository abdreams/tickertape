...
const StockDetails = () => {
  ...
  const [stockData, setStockData] = useState(dummyStockDetails);  // Use dummy data for now
  const [timeRange, setTimeRange] = useState('1week');

  useEffect(() => {
    const fetchStockDetails = async () => {
      // Dummy fetch to mimic API call
      // Replace this with actual API call
      // const response = await fetch(`https://dummyapi.io/data/api/stockdetails/${stocksymbol}`);
      // const result = await response.json();
      // setStockData(result);

      // Use dummy data for now
      setStockData(dummyStockDetails);
    };
